...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    try:
        ...
        for provider, choices in model_choices.items():
            ...
            if isinstance(saved_model, str) and (provider == "openai" or saved_model in choices):
    ...
    async def configure_provider_model(
        ...
        if provider != "openai" and request.model not in model_choices[provider]:
        ...
        temporary.write_text(
            json.dumps(model_preferences, ensure_ascii=False, indent=2), encoding="utf-8"
        )
    ...
    @app.post("/providers/openai/models/discover")
    async def discover_openai_models(
        ...
        api_key = supplied_key or load_api_key("openai")
        if not api_key:
