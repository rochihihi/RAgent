...
def _run_upgrade_check(check_id: str) -> dict[str, Any]:
    ...
    try:
        if check_id == "task_contract":
            ...
            evidence.append("已正确提取目标文件、保护文件、新功能和保留行为")
...
def _detached_process_is_running(command: list[str]) -> bool:
    rendered = " ".join(command)
    candidates = re.findall(r"[^\s'\"]+\.(?:py|go|exe)", rendered, flags=re.IGNORECASE)
    target = Path(candidates[-1].rstrip(",;)")).name if candidates else ""
    ...
    try:
        ...
        return False
    return target.casefold() in result.stdout.casefold()
...
def create_studio_router(settings: Settings) -> APIRouter:
    ...
    async def studio_brand() -> Path:
        ...
        return root / "assets" / "ragent-logo.jpg"
    ...
    async def run_upgrade_check(check_id: str) -> dict[str, Any]:
        if check_id == "all":
            ...
            return {"version": "0.3.0", "results": results}
        if check_id not in UPGRADE_CHECKS:
    ...
    async def list_sessions() -> list[dict[str, Any]]:
        return [session.model_dump(mode="json") for session in store.list_sessions()]

    @router.get("/studio-api/sessions/{session_id}")
    async def get_session(session_id: str) -> dict[str, Any]:
        return load_session(session_id).model_dump(mode="json")
    ...
    async def session_files(session_id: str) -> dict[str, list[str]]:
        session = load_session(session_id)
        return {"files": _file_tree(Path(session.repo_root))}
    ...
    async def execute(
        ...
        *,
        ...
        model_settings = replace(
            settings,
            model=session.model if session.provider == "openai" else settings.model,
            deepseek_model=(
    ...
    async def decide_permission(
        ...
        if request.approved:
            if pending.decision is not None:
                ...
                grants.append(fingerprint(decision))
            ...
            if pending.access == "execute" and pending.command:
                ...
                try:
                    ...
                    if pending.operation == StudioAction.START_TERMINAL.value:
                        ...
                        outcome = await asyncio.to_thread(
                            runner.launch if is_detached_launch(pending.command) else runner.run,
                            pending.command,
                        )
                except Exception as exc:
                    session.pending_permission = None
                ...
                if outcome is None:
                    ...
                    observation = StudioObservation(
                        kind="command",
                        summary=(
                            "已执行允许的命令。" if outcome.passed else "允许的命令执行未通过。"
                        ),
                ...
                if (
                    ...
                    and outcome.passed
                    and StudioAgent.is_verification_command(pending.command)
                ):
                ...
                if install_launch and pending.follow_up_command:
                    ...
                    store.save(session, "assistant_message", {"content": message})
                    return {"status": "approved"}
                detached_launch = is_detached_launch(pending.command)
                if detached_launch:
                    compound_launch = bool(
                    ...
                    if compound_launch:
                        # The approved launch command has already executed and
                        # its observation is durable.  If that satisfies the
                        # remaining contract, finish from local evidence now;
                        # asking the model to plan again caused repeated open
                        # commands, repeated permission dialogs, and false
                        # "still running" states.
                        unmet = StudioAgent._validate_task_contract(session)
                        if outcome.passed and not unmet:
                            launched_target = next(
                                (
                                    part
                                    for part in reversed(pending.command)
                            ...
                            session.failure_reason = None
                            session.pause_reason = None
                            for plan_item in session.plan:
                                plan_item.status = PlanStatus.COMPLETED
                            message = StudioAgent._completion_message(
                                session,
                                pending.command,
                                suffix=f"修改已写入当前工作区，且已打开 {launched_target}。",
                            )
                            session.messages.append(
                                StudioMessage(role="assistant", content=message)
                            )
                            store.save(
                                session,
                                "completed",
                                {
                                    "summary": message,
                                    "launch_command": pending.command,
                        ...
                        task = asyncio.create_task(
                            ...
                            name=f"studio-{session_id}-permission-launch",
                        )
                        tasks[session_id] = task
                        return {"status": "approved"}
                    session.status = "idle"
                    session.activity = "idle"
                    web_launch = any(
                        part.casefold().endswith((".html", ".htm")) for part in pending.command[4:]
                    )
                    running = outcome.passed and await asyncio.to_thread(
                        _detached_process_is_running, pending.command
                    )
                    running = running or "window_confirmed=true" in outcome.stdout.casefold()
                    if running:
                        message = "程序已打开。"
                    elif outcome.passed and web_launch:
                        message = "网页已打开。"
                    elif outcome.passed:
                        message = (
                            "启动命令已结束，但未确认目标程序仍在运行；"
                            "为避免反复弹窗，RAgent 不会自动重试。"
                        )
                    else:
                        message = "打开失败，详情见右侧工具结果。"
                    session.messages.append(StudioMessage(role="assistant", content=message))
                    store.save(session, "completed", {"summary": message})
                    return {"status": "approved"}
