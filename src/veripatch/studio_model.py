...
_NATIVE_TOOL_FIELDS: dict[str, dict[str, dict[str, Any]]] = {
    ...
    "request_permission": {"path": {"type": "string"}},
...
def _native_tools(*, responses_api: bool) -> list[dict[str, Any]]:
...
the workspace, use request_permission with the exact absolute path and a concise rationale; never
assume approval. Use respond for questions that need no tools. Use finish only when the user's task
...
- request_permission: action,rationale,path
- respond, finish, or fail: action,rationale,message
Use create/edit instead of pasting code in message or asking the user to save it. After changing a
file, run a suitable verification action, then finish with a concise result. Do not claim tools are
...
class StudioProviderModel:
    ...
    def __init__(
        ...
        *,
        ...
        self.base_url = (
            _openai_base_url(settings).rstrip("/")
            if provider == "openai"
        ...
        if provider == "deepseek":
            ...
            kwargs["base_url"] = _openai_base_url(settings)
    ...
    async def _request_decision(
        ...
        if self.provider == "openai":
            ...
            if not is_official_openai or _OPENAI_PROTOCOL_CACHE.get(self.base_url) == "chat":
                response, decision = await self._request_openai_chat(compatible_context, response_instructions)
            else:
                ...
                response = await self.client.responses.create(
                    ...
                    input=prompt,
                    tools=_native_tools(responses_api=True),
    ...
    async def _request_openai_chat(self, compatible_context: str, response_instructions: str = "") -> tuple[Any, StudioDecision]:
        ...
        if use_tools:
            kwargs["tools"] = _native_tools(responses_api=False)
    ...
    def _raw_compatible_chat(self, payload: dict[str, Any]) -> SimpleNamespace:
        ...
        with httpx.stream(
            ...
            return self._decode_compatible_stream(response)

    def _decode_compatible_stream(self, response: httpx.Response) -> SimpleNamespace:
