...
def _without_negated_mutations(message: str) -> str:
    normalized = re.sub(
        r"(?i)(?:不要|不得|无需|不需要|不允许|禁止|不是让你|不是要你|别|不)\s*"
        r"(?:修改|改动|改|编辑|写入|删除|创建|新建)[^，。；;\n]*",
        "",
        message,
    )
    return re.sub(
...
def requests_code_change(message: str) -> bool:
    ...
    return bool(
        re.search(
            ...
            r"改(?!完|好|过|后|进)|改进|重写|重新写|编写|创建|新建|制作|"
            r"写(?!完|好|过)|做(?!了?什么|完|好|过)|生成|转换|修复|优化|升级|"
            r"添加|加一行|新增|删除|实现|开发|迁移|重构|调整|完善|美化|"
...
def classify_intent(message: str) -> IntentPolicy:
    mutation_denied = bool(
        re.search(
            r"(?i)(?:不要|不得|无需|不需要|不允许|禁止|不是让你|不是要你|别|不)\s*"
            r"(?:修改|改动|编辑|写入|删除|创建|新建|改)\s*"
            r"(?:(?:任何|任意|所有|全部)\s*)?(?:文件|代码|内容|东西)?"
            r"(?=[，。；;\n]|$)"
            r"|\b(?:do\s+not|don't|must\s+not|without)\s+"
            r"(?:modify|edit|change|write|delete|create)\s+(?:anything|any\s+files?|files?)\b",
            message,
        )
    )
    command_denied = bool(
    ...
    mutation = requests_code_change(message) and not mutation_denied
