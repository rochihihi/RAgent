...
class TerminalRegistry:
    ...
    def start(
        ...
        *,
        ...
        return {"terminal_id": terminal_id, "pid": process.pid, "running": True, "output": ""}
    ...
    def poll(self, root: Path, terminal_id: str) -> dict[str, object]:
        return self._get(root, terminal_id).poll()
...
def inspect_visible_processes(query: str = "") -> list[dict[str, object]]:
    ...
    callback = ctypes.WINFUNCTYPE(ctypes.c_bool, ctypes.c_int, ctypes.c_int)(collect)
...
class SafeStudioCommandRunner:
    ...
    def launch(self, command: list[str]) -> TestOutcome:
        ...
        # A compiler/launcher process can remain alive briefly even when the
        # produced GUI immediately fails.  Success therefore requires a real
        # visible top-level window, not merely a PID returned by Popen.
        ...
        while time.monotonic() < deadline:
            ...
            if _visible_window_handles() - windows_before:
        ...
        return TestOutcome(
            command=command,
            exit_code=0 if window_confirmed else (exit_code if exit_code is not None else 1),
            stdout=(
                f"Started process {process.pid}; window_confirmed=true"
                if window_confirmed
                else f"Process {process.pid} did not create a visible window"
            ),
            stderr="" if window_confirmed else "未检测到目标程序的可见窗口。",
            duration_seconds=time.perf_counter() - started,
        )
