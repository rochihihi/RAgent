...
export const api = {
  ...
  files: (id: string) => request<{ files: string[] }>(`/studio-api/sessions/${id}/files`),
  ...
  discoverModels: (base_url: string, api_key?: string) => request<{ models: string[] }>("/providers/openai/models/discover", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ base_url, api_key: api_key || null }) }),
