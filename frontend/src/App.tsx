...
export function App() {
  ...
  const refresh = useCallback(async () => {
    ...
    setActive(session); setEvents(nextEvents); setFiles(tree.files);
  ...
  return <div className="app-shell">
    ...
    <div className={`workspace${showTrace ? " with-trace" : ""}${showSidebar ? " with-sidebar" : ""}`} style={{ "--left-pane": `${leftPane}px`, "--right-pane": `${rightPane}px` } as CSSProperties}>
      <aside className="sidebar">
        <div className="side-actions"><button className="primary" onClick={() => setModal("new")}>＋ 新建对话</button><button className="icon" title="Git 工作区" disabled={!active} onClick={() => setModal("git")} aria-label="Git 工作区"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" aria-hidden="true"><circle cx="6" cy="5" r="2.5"/><circle cx="6" cy="19" r="2.5"/><circle cx="18" cy="6" r="2.5"/><path d="M6 7.5v9M18 8.5v1a6 6 0 0 1-6 6H6"/></svg></button><button className="icon" title="升级验收" onClick={() => setModal("acceptance")}>✓</button><button className="icon" title="API 配置" onClick={() => setModal("api")}>⚙</button></div>
        ...
        <section className="project-panel">
          <header><div><span>项目文件</span><strong>{active ? projectName(active.repo_root) : "尚未选择仓库"}</strong></div><b>{files.length}</b></header>
          <div className="file-tree">{files.map((path) => <button key={path} onClick={() => openFile(path)} className={changed.has(path) ? "changed" : ""}><i>{fileGlyph(path)}</i><span><strong>{path.split("/").pop()}</strong><small>{fileParent(path)}</small></span>{changed.has(path) && <em>已修改</em>}</button>)}</div>
        </section>
      </aside>
      <div className="pane-resizer left" onPointerDown={(event) => resizePane("left", event)} />
      <main className="conversation">
        <header className="hero" key={`hero-${activeId || "empty"}`}><button aria-label="切换侧栏" aria-expanded={showSidebar} onClick={() => setShowSidebar(!showSidebar)}>☰</button><div className="conversation-heading"><h1>{active?.title || "开始一个编码任务"}</h1><p>{active ? `${active.provider} · ${active.model} · ${active.verification_mode}` : "选择项目，然后用自然语言要求 Agent 阅读、修改并验证代码。"}</p></div><button aria-expanded={showTrace} onClick={() => setShowTrace(!showTrace)}>执行记录</button>{active && <div className="conversation-actions"><button disabled={active.status === "running" || active.status === "waiting_permission"} onClick={() => setModal("skills")}>技能</button><button onClick={() => setModal("settings")}>会话设置</button></div>}</header>
        <section ref={messagesRef} onScroll={trackMessageScroll} className="messages" key={`messages-${activeId || "empty"}`}>
          {visibleMessages.length ? visibleMessages.map((message, index) => <article className={message.role} style={{ "--message-index": Math.min(index, 8) } as CSSProperties} key={`${message.role}-${index}`}><MessageContent content={message.content} /></article>) : <div className="welcome"><img src="/studio-brand" /><h2>把代码交给一个会验证结果的 Agent</h2><p>创建会话后，可以说“解释这个项目”或“修复失败的测试”。</p></div>}
        ...
        <form className="composer" onSubmit={send}><div className="compose-card"><textarea value={draft} onChange={(e) => setDraft(e.target.value)} placeholder={active?.status === "running" ? "输入纠正或追加要求，将在安全边界切换…" : "给 RAgent 一个任务…"} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); e.currentTarget.form?.requestSubmit(); } }} /><button disabled={!active}>{active?.status === "running" ? "↪" : "↑"}</button><ComposerMeta session={active} onChanged={refresh} /></div></form>
      </main>
      <div className="pane-resizer right" onPointerDown={(event) => resizePane("right", event)} />
      <aside className="trace"><header><span className="event-count">{activityEvents.length} 条记录</span><span className={`state ${active?.status || "idle"}`}><i />{activityText[active?.activity || active?.status || "idle"] || "待命"}</span></header><div className="timeline">{activityEvents.slice().reverse().map((item, index) => <details className={`event-card event-${item.event_type}`} style={{ "--event-index": Math.min(index, 7) } as CSSProperties} key={item.sequence} open={["failed", "paused"].includes(item.event_type)}><summary><i>{eventGlyph(item.event_type)}</i><span><strong>{actionNames[item.event_type] || item.event_type}</strong><small>{summary(item)}</small><time>{new Date(item.created_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" })}</time></span></summary><pre>{JSON.stringify(item.payload, null, 2)}</pre></details>)}</div></aside>
    </div>
...
function RuntimeDiagnostic({ session, events }: { session: Session; events: Event[] }) {
  ...
  const modelRelated = diagnostic || retry || /model|模型|timeout|protocol|http|connect|attribute/i.test(failure);
  ...
  return <article className="runtime-diagnostic" aria-live="polite">
    <header><span>!</span><div><strong>任务已安全暂停</strong><small>代码与执行进度已经保存，不是项目文件突然丢失</small></div><b>{source} · {confidence}置信度</b></header>
...
function summary(event: Event) { const p = event.payload; if (event.event_type === "model_response" && Array.isArray(p.tool_names)) { const names = p.tool_names.join(" → "); const targets = Array.isArray(p.tool_targets) ? p.tool_targets.filter(Boolean).join("；") : ""; return `工具：${names}${targets ? ` · 目标：${targets}` : ""} · ${String(p.protocol || "unknown")}`; } return String(p.summary || p.rationale || p.reason || "状态已保存"); }
function projectName(path: string) { return path.replace(/[\\/]+$/, "").split(/[\\/]/).pop() || path; }
function fileParent(path: string) { const parts = path.split("/"); parts.pop(); return parts.join("/") || "项目根目录"; }
function fileGlyph(path: string) { const extension = path.split(".").pop()?.toLowerCase(); if (["py", "js", "ts", "tsx", "jsx"].includes(extension || "")) return "⌘"; if (["json", "toml", "yaml", "yml", "ini"].includes(extension || "")) return "◆"; if (["md", "txt", "rst"].includes(extension || "")) return "≡"; return "·"; }
function eventGlyph(type: string) { if (type === "failed") return "!"; if (type === "completed") return "✓"; if (type === "user_message") return "↑"; if (type === "observation") return "↳"; if (type === "decision") return "◇"; return "·"; }
...
function EffortSlider({ current, options, onCommit }: { current: Session; options: Array<[string, string]>; onCommit: (value: string) => Promise<void> }) {
  ...
  const [localIndex, setLocalIndex] = useState(serverIndex);
  useEffect(() => { setLocalIndex(serverIndex); }, [current.session_id, current.provider, current.reasoning_effort, serverIndex]);
  async function commit(index: number) { const selected = options[index]?.[0]; if (selected && selected !== current.reasoning_effort) await onCommit(selected); }
  return <label className="effort-control">推理 <b>{options[localIndex]?.[1]}</b><input type="range" min="0" max={options.length - 1} value={localIndex} disabled={current.status === "running"} onChange={(e) => setLocalIndex(Number(e.target.value))} onPointerUp={(e) => commit(Number(e.currentTarget.value))} onPointerCancel={(e) => commit(Number(e.currentTarget.value))} onKeyUp={(e) => commit(Number(e.currentTarget.value))} onBlur={(e) => commit(Number(e.currentTarget.value))} /></label>;
}
...
function NewSession({ onClose, onCreated }: { onClose: () => void; onCreated: (id: string) => void }) {
  ...
  useEffect(() => { Promise.all([api.providers(), api.models()]).then(([states, models]) => { const next: Provider = states.deepseek.configured || !states.openai.configured ? "deepseek" : "openai"; setProvider(next); setModel(models[next].selected || models[next].choices[0]); }).catch((e) => setError(e.message)); }, []);
  async function submit(e: FormEvent<HTMLFormElement>) { e.preventDefault(); const data = new FormData(e.currentTarget); try { const result = await api.createSession({ repo_root: path, provider, model, reasoning_effort: provider === "openai" ? "medium" : "high", permission_mode: data.get("permission_mode"), verification_mode: data.get("verification_mode"), test_command: String(data.get("test_command") || "") }); onCreated(result.session_id); } catch (e) { setError((e as Error).message); } }
...
function ApiSettings({ onClose }: { onClose: () => void }) {
  ...
  async function refreshQuotas() { setRefreshing(true); try { const results = await Promise.allSettled([api.quota("deepseek"), api.quota("openai")]); const next = { deepseek: results[0].status === "fulfilled" ? results[0].value : quotas.deepseek, openai: results[1].status === "fulfilled" ? results[1].value : quotas.openai }; const now = Date.now(); setQuotas(next); setCheckedAt(formatQuotaTime(now)); localStorage.setItem("ragent_quota_cache", JSON.stringify({ checkedAt: now, quotas: next })); } finally { setRefreshing(false); } }
  ...
  async function discover() { try { const result = await api.discoverModels(base, key); setModels((m) => m ? { ...m, openai: { selected: result.models[0], choices: result.models } } : m); setSelected(result.models[0]); setNotice(`读取到 ${result.models.length} 个模型`); } catch (e) { setNotice((e as Error).message); } }
  async function save() { try { if (key) await api.saveKey(provider, key); if (provider === "openai") await api.saveOpenAI(base, selected, quotaToken, quotaRefreshToken, quotaUserId); else await api.saveModel(provider, selected); setNotice("配置已保存"); setKey(""); setQuotaToken(""); setQuotaRefreshToken(""); await load(); } catch (e) { setNotice((e as Error).message); } }
  ...
  return <ModalCard onClose={onClose}><ModalHead title="API 配置" onClose={onClose} /><section className="quota-panel"><header><div><strong>额度概览</strong><small>{checkedAt ? `最近刷新 ${checkedAt}` : "打开设置不会立即请求额度"}</small></div><nav className="quota-actions"><label className="quota-auto"><input type="checkbox" checked={autoRefresh} onChange={(e) => setAutoRefresh(e.target.checked)} /><span>每 5 分钟</span></label><button className={`quota-refresh ${refreshing ? "refreshing" : ""}`} disabled={refreshing} onClick={() => refreshQuotas()}><i>↻</i><span>{refreshing ? "读取中" : "刷新"}</span></button></nav></header><div>{(["deepseek", "openai"] as Provider[]).map((name) => <article key={name}><span>{name === "deepseek" ? "DeepSeek" : "OpenAI / 中转站"}</span><strong>{quotaText(quotas[name])}</strong><small>{quotaHint(quotas[name])}</small></article>)}</div></section><div className="provider-tabs"><button className={provider === "deepseek" ? "active" : ""} onClick={() => { setProvider("deepseek"); setTestResult(null); }}>DeepSeek</button><button className={provider === "openai" ? "active" : ""} onClick={() => { setProvider("openai"); setTestResult(null); }}>OpenAI</button></div><p className="credential-state">{states?.[provider]?.configured ? "● 已配置" : "○ 尚未配置"}</p><label>API Key<input type="password" value={key} onChange={(e) => setKey(e.target.value)} placeholder={states?.[provider]?.configured ? "输入新密钥可更新" : "粘贴 API Key"} /></label>{provider === "openai" && <><label>兼容 Base URL<div className="path-row"><input value={base} onChange={(e) => setBase(e.target.value)} /><button onClick={discover}>读取模型</button></div></label><section className="quota-credentials"><header><div><strong>Sub2API 账户额度</strong><small>令牌过期后自动续期并继续查询</small></div><span>{states?.openai.quota_refresh_configured ? "自动续期" : states?.openai.quota_configured ? "已配置" : "可选"}</span></header><label>auth_token<input type="password" value={quotaToken} onChange={(e) => setQuotaToken(e.target.value)} placeholder={states?.openai.quota_configured ? "输入新令牌可更新" : "localStorage 中的 auth_token"} /></label><label>refresh_token<input type="password" value={quotaRefreshToken} onChange={(e) => setQuotaRefreshToken(e.target.value)} placeholder={states?.openai.quota_refresh_configured ? "已加密保存，输入可更新" : "localStorage 中的 refresh_token"} /></label><label>NewAPI 用户 ID（Sub2API 留空）<input value={quotaUserId} onChange={(e) => setQuotaUserId(e.target.value)} placeholder="仅 NewAPI 需要，例如 1" /></label></section></>}<label>默认模型<select value={selected} onChange={(e) => { setSelected(e.target.value); setTestResult(null); }}>{choices.map((m) => <option key={m}>{m}</option>)}</select></label><section className={`connection-test ${testResult ? (testResult.ok ? "success" : "failure") : ""}`}><div><strong>连接测试</strong><small>使用当前密钥、地址和模型发送一个最小请求</small></div><button type="button" disabled={testing || !selected} onClick={() => void testConnection()}>{testing ? "测试中…" : "测试是否可用"}</button>{testResult && <p>{testResult.text}</p>}</section>{notice && <p className="notice">{notice}</p>}<div className="form-actions"><button onClick={onClose}>关闭</button><button className="primary" onClick={save}>保存配置</button></div></ModalCard>;
}

function quotaText(quota?: Quota) { if (!quota) return "未查询"; if (!quota.configured) return "未配置"; if (!quota.supported) return "平台查看"; if (quota.error?.includes("未提供")) return "未开放余额接口"; if (quota.error?.includes("后台访问令牌")) return "需要后台令牌"; if (quota.error || quota.is_available === false) return "查询失败"; const balance = quota.balances[0]; return balance ? `${balance.currency} ${balance.total_balance}` : "可用"; }
