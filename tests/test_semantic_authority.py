...
def test_classifier_failure_is_not_presented_as_unclear_user_authority(tmp_path):
    class UnavailableModel:
        async def classify_intent(self, messages, message):
            raise RuntimeError("semantic service unavailable")

        async def decide(self, *args, **kwargs):
            pytest.fail("Execution must not start after failed semantic classification")
    ...
    agent = StudioAgent(UnavailableModel(), StudioStore(tmp_path / "state.sqlite3"))
    asyncio.run(agent.handle(session, "把 a.txt 重命名为 b.txt。"))
    assert "语义解析调用失败" in session.messages[-1].content
    assert "授权不够明确" not in session.messages[-1].content
    assert StudioAction.MOVE_FILE.value not in session.task_contract.allowed_actions
