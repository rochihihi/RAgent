...
from veripatch.studio_agent import StudioAgent, _requests_code_change
...
def test_read_only_contract_blocks_model_mutation_and_commands(tmp_path: Path) -> None:
...
def test_explicit_change_authority_overrides_semantic_uncertainty(
    ...
    assert model.classifier_calls == 1
    assert result.activity != "waiting_user"
    assert result.task_contract is not None
...
def test_semantic_classifier_refines_deliberative_question_without_write(tmp_path: Path) -> None:
    ...
    assert result.task_contract.intent_source == "model+deterministic"
    assert StudioAction.EDIT.value not in result.task_contract.allowed_actions


def test_semantic_classifier_is_primary_for_clear_substantive_requests(tmp_path: Path) -> None:
    ...
    assert model.calls == 1
    assert result.task_contract is not None
    assert result.task_contract.dialogue_act == "analyze_and_report_status"
    assert result.task_contract.objectives == ["分析 hello.py", "报告现有验证状态"]
    assert result.task_contract.questions == ["是否存在测试通过证据"]
    assert result.task_contract.requested_actions == ["read", "respond"]
    assert result.task_contract.intent_source == "model+deterministic"
...
def test_user_message_is_persisted_before_semantic_classifier_finishes(
    ...
    async def scenario() -> StudioSession:
        ...
        task = asyncio.create_task(StudioAgent(model, store).handle(session, "分析项目结构"))
        await model.started.wait()
        persisted = store.load(session.session_id)
        assert persisted is not None
        assert persisted.status == "running"
        assert persisted.messages[-1].role == "user"
        assert persisted.messages[-1].content == "分析项目结构"
        model.release.set()
        return await task

    result = asyncio.run(scenario())

    assert [item.content for item in result.messages].count("分析项目结构") == 1
...
def test_verified_changes_complete_automatically_when_turn_budget_ends(tmp_path: Path) -> None:
    ...
    assert "完整测试已通过" in result.messages[-1].content
    assert "python -m pytest" not in result.messages[-1].content
    assert "完成内容\n- 修复实现" in result.messages[-1].content
...
def test_read_only_answer_is_not_misclassified_and_continue_does_not_restart(
    ...
    assert answered.task_contract.intent == "answer"
...
def test_budget_extends_only_after_meaningful_progress(tmp_path: Path) -> None:
    ...
    result = asyncio.run(StudioAgent(model, store, max_steps=40).handle(session, "修改并升级项目"))
...
def test_previously_approved_open_request_finishes_without_model_loop(tmp_path: Path) -> None:
    (tmp_path / "index.html").write_text("<html><body>ok</body></html>", encoding="utf-8")
    ...
    runner = RunnerOutcome(command=command, exit_code=0, stdout="", stderr="", duration_seconds=0)

    with patch("veripatch.studio_tools.SafeStudioCommandRunner.launch", return_value=runner):
        result = asyncio.run(
            StudioAgent(ModelMustNotBeCalled(), StudioStore(tmp_path / "launch.sqlite3")).handle(
                session, "帮我打开"
            )
        )
    ...
    assert result.messages[-1].content == "已打开 index.html。"
    assert not any(item.kind == "duplicate_action" for item in result.observations)
...
def test_previously_approved_gui_launch_without_window_is_not_reported_as_open(
    ...
    with patch("veripatch.studio_tools.SafeStudioCommandRunner.launch", return_value=runner):
        result = asyncio.run(
            StudioAgent(ModelMustNotBeCalled(), StudioStore(tmp_path / "no-window.sqlite3")).handle(
                session, "再打开一次"
            )
        )

    assert result.status == "idle"
    assert result.messages[-1].content == "未检测到 calculator.py 的可见窗口，启动没有成功。"
...
def test_verified_work_recovers_when_final_model_summary_disconnects(tmp_path: Path) -> None:
    ...
    result = asyncio.run(
        StudioAgent(VerifiedThenDisconnectedModel(), store).handle(session, "创建并验证脚本")
    )
    ...
    assert result.review_completed is True
    assert result.failure_reason is None
    assert "模型总结连接中断" in result.messages[-1].content
    assert "本地修改和验证证据完整" in result.messages[-1].content
    assert store.events(session.session_id)[-1]["event_type"] == "recovered_completion"
...
def test_permission_request_under_approved_parent_is_reused_without_prompt(
    tmp_path: Path,
) -> None:
    ...
    assert terminal is False
    assert session.pending_permission is None
    assert session.observations[-1].kind == "permission_reused"
    assert session.observations[-1].payload["approved_root"] == str(approved)
...
def test_studio_api_executes_an_approved_detached_launch_and_stops(
    ...
    monkeypatch.setattr("veripatch.studio_api._detached_process_is_running", lambda _: True)
    ...
    assert completed.messages[-1].content == "程序已打开。"
...
def test_studio_api_completes_compound_task_after_approved_launch(
    ...
    session.observations.append(
        StudioObservation(
            ...
            payload={"path": "calculator.html", "intent": "创建图形计算器"},
        )
    )
    store.save(session, "permission_requested", session.pending_permission.model_dump())
    monkeypatch.setattr(
...
def test_unverified_launch_stops_without_automatic_retry(
    ...
    monkeypatch.setattr("veripatch.studio_api._detached_process_is_running", lambda _: False)
    ...
    assert completed is not None
    assert completed.status == "idle"
    assert "不会自动重试" in completed.messages[-1].content
...
def test_studio_api_serves_workspace_and_validates_models(tmp_path: Path) -> None:
    ...
    assert brand.headers["content-type"] == "image/jpeg"
    ...
    assert "把代码交给一个会验证结果的 Agent" in asset.text
...
def test_upgrade_acceptance_checks_run_independently(tmp_path: Path) -> None:
    ...
    assert catalog.json()["version"] == "0.3.0"
